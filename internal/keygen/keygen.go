...
func EnsureKeyPair(out io.Writer) (KeyPair, error) {
	...
	// Ignore write error to out — it is informational only and must not
	// fail the key generation that already succeeded.
	_, _ = fmt.Fprintf(out, "✓ generated new SSH key: %s\n", kp.PrivatePath)
...
func InstallOnServer(pubKeyPath string, runner func(cmd string) (string, error)) error {
	pubData, err := os.ReadFile(pubKeyPath) //nolint:gosec
	if err != nil {
...
func generate(kp KeyPair) error {
	...
	// Public key uses 0644 — it is not secret and must be readable by sshd.
	if err = os.WriteFile(kp.PublicPath, ssh.MarshalAuthorizedKey(sshPub), 0o644); err != nil { //nolint:gosec
